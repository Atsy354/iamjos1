...
const PLUGIN_CATALOG: PluginItem[] = [
  { id: "custom-block", name: "Custom Block Manager", description: "Kelola blok konten di sidebar.", enabled: true },
  { id: "google-analytics", name: "Google Analytics", description: "Tambahkan tracking Analytics.", enabled: false },
  { id: "crossref", name: "Crossref XML Export", description: "Ekspor metadata artikel ke Crossref XML.", enabled: true },
  { id: "doaj", name: "DOAJ Export Plugin", description: "Ekspor metadata ke DOAJ.", enabled: false },
];

export async function getSitePlugins(): Promise<PluginItem[]> {
  try {
    const supabase = getSupabaseAdminClient();
    const { data, error } = await supabase.from("site_plugins").select("id,enabled");
    if (error) {
      return PLUGIN_CATALOG;
    }
    const states = new Map<string, boolean>();
    for (const row of data ?? []) {
      states.set(String((row as { id: string }).id), Boolean((row as { enabled?: boolean }).enabled));
    }
    return PLUGIN_CATALOG.map((p) => ({ ...p, enabled: states.has(p.id) ? Boolean(states.get(p.id)) : p.enabled }));
  } catch {
    return PLUGIN_CATALOG;
  }
}
...
export async function getSiteNavigation(): Promise<SiteNavigation> {
  try {
    ...
    const parsed = navigationSchema.safeParse({
      primary: (data as { primary?: string[] }).primary ?? ["Home", "About", "Login", "Register"],
      user: (data as { user?: string[] }).user ?? ["Dashboard", "Profile", "Logout"],
    });
    if (!parsed.success) {
      return { primary: ["Home", "About", "Login", "Register"], user: ["Dashboard", "Profile", "Logout"] };
    }
...
export async function updateSiteNavigationAction(formData: FormData): Promise<UpdateResult> {
  try {
    ...
    const { error } = await supabase
      .from("site_navigation")
      .upsert({ id: "site", primary: values.data.primary, user: values.data.user }, { onConflict: "id" });
    if (error) {
